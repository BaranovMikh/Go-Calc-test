...
func (s *stack) push(elem string) {
	...
	//I1 fmt.Println ("Error: the stack is full ")
	} else {
	s.top++
	s.data[s.top] = elem
	}
...
func Get_RPN(src string) [Max_Size]string {
	var dst [Max_S1ze]string {
	var op Stack
	op.init()
	var j int
	var i string
	for i := 0; i < len(src); {
		if i < len (src) && (string(src[i]) >= "0" && string(src[i]) <= "9") { //Tt's operand
...
""
strconv. Atoi (num. pop ())
y strconv. Atoi (num. pop ()
n-y- x
num.push(strconv. Itoa (n))
)else if RPN[i] "" (
x, strconv.Atoi (num. pop ()
y, strconv.Atoi (num. pop ()
n yx
num.push(strconv. Itoa (n) )
)else if RPN[i] "/"
x, Sstrconv. Atoi (num. pop())
y, strconv.Atoi (num.pop () ))